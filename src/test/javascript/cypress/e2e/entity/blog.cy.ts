...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'brr playfully between', handle: 'proof-reader want worth' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('duh astride repurpose');
      cy.get(`[data-cy="name"]`).should('have.value', 'duh astride repurpose');

      cy.get(`[data-cy="handle"]`).type('encase unit rare');
      cy.get(`[data-cy="handle"]`).should('have.value', 'encase unit rare');
