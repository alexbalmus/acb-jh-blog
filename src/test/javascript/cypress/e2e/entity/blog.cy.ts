...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'speakerphone', handle: 'spelt' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('until commission except');
      cy.get(`[data-cy="name"]`).should('have.value', 'until commission except');

      cy.get(`[data-cy="handle"]`).type('mysterious');
      cy.get(`[data-cy="handle"]`).should('have.value', 'mysterious');
